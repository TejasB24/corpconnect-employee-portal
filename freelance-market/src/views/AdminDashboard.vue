<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid lg:grid-cols-[260px_1fr] gap-6">
    ...
    <aside class="bg-white border border-gray-200 rounded-xl p-5 h-max">
      ...
      <nav class="space-y-2">
        <a href="#overview" class="block px-3 py-2 rounded-md hover:bg-gray-50">Overview</a>
        <a href="#teams" class="block px-3 py-2 rounded-md hover:bg-gray-50">Teams</a>
        <a href="#reports" class="block px-3 py-2 rounded-md hover:bg-gray-50">Reports</a>
      </nav>
    </aside>
    ...
    <section class="space-y-6">
      <!-- Overview -->
      <div id="overview" class="bg-white border border-gray-200 rounded-xl p-6">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold">Organization overview</h2>
        ...
        <div v-else class="mt-4">
          <div class="font-medium">Welcome, {{ user?.name || '—' }} <span class="text-xs text-gray-500">({{ user?.role || '—' }})</span></div>
          <p class="text-sm text-gray-600 mt-1">Manage teams, review reports, and oversee projects.</p>
        </div>
      </div>

      <!-- Teams (using projects endpoint as placeholder) -->
      <div id="teams" class="bg-white border border-gray-200 rounded-xl p-6">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold">Active projects (as teams)</h2>
        ...
        <div v-else class="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div v-if="projects.length === 0" class="text-gray-600">No active items.</div>
        </div>
      </div>

      <!-- Reports (using messages endpoint as placeholder) -->
      <div id="reports" class="bg-white border border-gray-200 rounded-xl p-6">
        <div class="flex items-center justify-between">
        ...
        <ul v-else class="mt-4 divide-y divide-gray-200">
          ...
          <li v-if="messages.length === 0" class="py-3 text-gray-600">No announcements.</li>
        </ul>
      </div>
    </section>
  </div>
...
async function fetchMessages() {
  ...
  try {
    ...
    messagesError.value = err?.response?.data?.message || err.message || 'Failed to load announcements'
    messages.value = []
  } finally {
