  ...
  <nav class="bg-white border-b border-gray-200 sticky top-0 z-40">
      ...
      <div class="flex h-16 items-center justify-between">
        <div class="flex items-center">
          <RouterLink to="/" class="text-xl font-bold text-primary"
            >Corp<span class="text-accent">Connect</span></RouterLink
          >
        </div>
    ...
    <div v-if="mobileOpen" class="md:hidden border-t border-gray-200">
      <div class="px-4 py-3 space-y-2">
        ...
        <RouterLink v-if="!isLoggedIn" @click="mobileOpen = false" to="/login" class="block py-2"
          >Login</RouterLink
        >
        <div v-else class="space-y-2">
          <RouterLink @click="mobileOpen = false" :to="dashboardPath" class="block py-2"
